...
class CameraApp(App):
    ...
    def on_texture_update(self, instance, texture):
        """標準Kivy Cameraの場合のみ使用 - camera4kivyでは自動処理"""
        if not self.camera4kivy_available and texture:
            try:
                # camera4kivyが利用できない場合のみ、簡易的な回転処理
                # 実際の回転はcamera4kivyの機能に任せる
                pass
            except Exception as e:
                print(f"Frame rotation error: {e}")
    
    def texture_to_frame(self, texture):
        """標準Kivy Cameraの場合のみ使用 - camera4kivyでは不要"""
        if not self.camera4kivy_available:
            # camera4kivyが利用できない場合のみ、簡易的な処理
            return None
        return None
